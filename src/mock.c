 ...
 * junk is only used here to make it conform
 * to the general format of functions in this file:
 * like Sersic().
 */
double
...
/* 
 * This function will find the moffat function 
 * alpha value based on the explantions here:
 * http://labs.adsabs.harvard.edu/adsabs/abs/2001MNRAS.328..977T/
 * When alpha=(fwhm/2)/(2^(1.0/beta)-1)^(0.5),
 * Then the moffat function at r is: (1.0 + (r/alpha)^2.0)^(-1.0*beta)
 */
double
...
/*
 * This approximation of b(n) for n>0.35 is taken from 
 *   McArthur, Courteau and Holtzman 2003:
...
sersic_b(double n)
{
    if(n<=0.35f) 
    {
        ...
        printf("\t\t\t Sersic n must be smaller than 0.35\n\n");
        exit(EXIT_FAILURE);
    }
...
/* 
 * Find the Sersic profile for a certain radius.
 * rdre=r/re, inv_n=1/n, nb= -1*b.
 */
double
 ...
 ****************************************************************/
/*
 * Calculate the Moffat profile on position (i,j) in a 
 * cartesian grid. While the ellipse of the profile
 * is rotated and has axis ratio q.
 * i,j: positions on the grid.
 * n,re,b: Moffat profile parameters.
 * c,s: Cosine and Sine of the ellipse position angle.
 * q: axis ratio of the ellipse.
 */
double
...
/*
 * This function is used in the integration of a 
 * moffat profile. It assumes a fixed y and integrates
...
/*
 * 2D integration of the Moffat profile:
 */
{
    gsl_function F;
    static double abserr;
        ...
        size_t *y_w, size_t *numpixs)
{
    ...
    totalflux=floatarrsum(*mock, *x_w * *y_w);
 ...
 ****************************************************************/
/*
 * Simple function to generate a seed 
 ...
 * distribution. Since the array might be two dimentional, 
 * the "array" pointer need not point to the first element. 
 * It just needs to point to the first element in a column. 
 * In case you want to fill a whole column of a 2D array, 
 * you point "array" to the first element in that column 
 * of the array, "num_rand" is the number of rows and 
 * "stride" is the number of clumns.
 * 
 * The output of gsl_rng_uniform gives results in the 
 * range [0,1). So by multiplying the result by
 * (range=higher_limit-lower_limit) we scale it to[0,range).
 * and finally by adding the lower limit to the random
 * variable, we ensure that the result is in the range:
...
/*
 * An already allocated array is input into
 * this function and is filled with random
 * Gaussians, with mode of zero and standard
 * deviation of sigma.
 */
...
setprflprms(double **prflprms, size_t numprflprms, size_t *nummock,
        int size1, int size2, int hsize1, int hsize2)
{
    size_t i;  
    double *pprflprms;
    #if ONLYONEPROFILE
    float randparamranges[14]={0,0,0,0,4,4,25,25,
                               90,90,0.3,0.3,1,1};
    #else
    #if ONGRID
    size_t j, counter;
    #else
    #endif
    float randparamranges[14]={-10,+10,-5,+5,0.5,4,1,30,
                               0,360,0.2,1,0.005,0.05};
    /* For Some of bright faint objects 
    size_t num_mock=6;
    float randparamranges[14]={-20,+20,-20,+20,0.5,4,10,60,
                               0,360,0.2,1,0.1,1};*/
    #endif
    ...
    *nummock=25;
    #endif
    hsize1=hsize1; /* For non-grid cases. */
    hsize2=hsize2;

    #if ONLYONEPROFILE
    randparamranges[0]=size1/2;
    randparamranges[1]=size1/2;
    randparamranges[2]=size2/2;
    randparamranges[3]=size2/2;
    #else
    /* Note that incase the initial values are zero,
    the center of all the profiles will be in the output image. */
    ...
    *prflprms=malloc(numprflprms * *nummock*sizeof(double));
    ...
    for(i=2;i<numprflprms-1;i++)
        setunifrandvalues(&pprflprms[i], *nummock, numprflprms, 
                randparamranges[(i-2)*2], randparamranges[(i-2)*2+1]);

    #if ONGRID && !ONLYONEPROFILE
    counter=0;
    for(i=0;i<5;i++){
        for(j=0;j<5;j++)
        {
            pprflprms[counter*numprflprms+2]=hsize1+20+i*40;
            pprflprms[counter*numprflprms+3]=hsize2+20+j*40;
            pprflprms[counter*numprflprms+4]=0.5;
            pprflprms[counter*numprflprms+5]=1+(1+counter)/2;
            pprflprms[counter*numprflprms+6]=45;
            pprflprms[counter*numprflprms+7]=0.5;
            pprflprms[counter*numprflprms+8]=0.04/*0.005*(1+counter)*/;
            counter++;
        }
    }
    #endif
...
/* 
 * This function will make a mock profile and place it
 * in the larger image. 
 */
void
        ...
        float sky, float trunc, size_t hsize1, size_t hsize2)
{
    ...
    strcat(ai.c, "# 8: Signal to noise (average profile flux/sky).\n");    
    strcat(ai.c, "# 9: Total flux (Sky subtracted).\n\n"); 

    writeasciitable ("mock.txt", &ai, int_cols, accu_cols, space, prec);
}
 ...
 ****************************************************************
 ****************************************************************/
/* Put one or more mock profiles into and image,
    convolve it and add noise to the final result.
    The convolution is going to make the sides darker.
    So the actual image where the galaxies will be 
    placed is going to be larger than the desired image.
    After convolution those sides will be trimed and the
    pixels on the sides of the result will be equally
    convolved as the central pixels.

   Note on MINFLOAT: We are using float values here, due to
    roundoff errors, after convolution we might have pixel 
    values less than NUMMOCK, which are pure error, so
    ...
    *mock=calloc(nsize1*nsize2,sizeof(float));
    ...
    for(i=0;i<nummock;i++)
        profileinimg(*mock, nsize1, nsize2, trunc, integaccu, 
                ...
                sqrt(sky)*prflprms[i*numprflprms+8], /*aveflux=sigma*sn*/
    ...
    #if SAVEFITS
    array_to_fits("mock.fits", NULL, "NONOISE", "FLOAT", 
        *mock, size1, size2);
    #endif

    #if MOCKHIST
    histogram(*mock, size1*size2, MOCKHISTNUMBINS, 
        MOCKHISTMIN, MOCKHISTMAX, &nonoisehist);
    #endif

    addnoise(*mock, size1*size2, sky);

    #if SAVEFITS
    array_to_fits("mock.fits", NULL, "WITHNOISE", "FLOAT", 
        *mock, size1, size2);
    #endif

    #if MOCKHIST
    histogram(*mock, size1*size2, MOCKHISTNUMBINS, 
        MOCKHISTMIN, MOCKHISTMAX, &noisedhist);
    floatvmerge(nonoisehist, noisedhist, MOCKHISTNUMBINS, &allhist);
    printhist(allhist, "mockhists.txt", MOCKHISTNUMBINS, 3);
