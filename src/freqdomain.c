...
/* This function will receive a 2D complex array of type
    fftw_complex. It will then save the desired aspect of the complex
...
/* Saving a double array is hardly ever necessary: it will just take
    too much space and that level of accuracy is hardly ever
    needed. */
void
...
/* This function get two arrays: f(img) and h(psf) and convolves them,
    the result will be saved in c(convolved).  Space for c will be
...
/* This function will make the PSF based on the input and use that to
   convolve the image. For the parameters see mock.h.

   IMPORTANT NOTE: if *conv==NULL, it is assumed that you want to
   allocate space to it here. If not, it will assume that space is
   already allocated . */
void
        ...
        float integaccu, float s0_m1_g2, float **conv)
{
  ...
  if(CONVFUNCCHECK)
    {
      if(s0_m1_g2==0)
	sprintf(comment, "# Sersic profile, n=%.2f, re=%.2f, "
		"extended to %.2f*re\n", p1, p2, trunc);
      else if(s0_m1_g2==1)
	sprintf(comment, "# Moffat profile, FWHM=%.2f, "
		"beta=%.2f, extended to %.2f*FWHM\n",
		p2, p1, trunc);
      else if(s0_m1_g2==1)
  ...
  if(conv!=NULL)
    floatarrcpynomalloc(f, fsize1*fsize2, conv);
  else
