...
/*
 * Any ellipse can be enclosed into a rectangular
 * box. The purpose of this function is to give
 * the height and width of that box. 
 * 
 * The logic behind it is this:
 * All the points on the circumference of an ellipse that
 * is aligned on the x axis can be written as: 
 *
 * (acos(t),bsin(t)) where 0<t<2\pi.               (1)
 * 
 * But when we rotate the ellipse by \theta, the points 
 * can be characterized by:
 *
 * ( acos(t)cos(\theta)+bsin(t)sin(\theta),        (2)
 *  -acos(t)sin(\theta)+bsin(t)cos(\theta) )
 }
 * To find the maximum and minimum points of this function
 * you just have to take the derivative of each with 
 * respect to "t" and set it to zero. This will give you
 * the "t" that maximizes both x and the "t" that maximizes y.
 * Once you do that, you will get:
 *
 * For x: tan(t)=(b/a)tan(\theta)                  (3)
 * For y: tan(t)=(-b/a)cot(\theta)
 * 
 * once you find the "t", put it in (2) for the respective
 * coordinate and you will find the distance (about the 
 * center of the ellipse that encloses the whole ellipse.
 */
void
...
/* 
 * Find the correct size of the output image of an
 * elliptical profile and malloc the array to keep
 * the elliptical profile. Don't forget to free it later.
 * It is made for radial profiles, so it will fill the 
 * array with the distance to the center (x_c,y_c).
 * this will make applying a radial profile really easy.
 * This function is defined separately so that it can
 * be used for a wide variety of profiles.
 *
 * Input parameters:
...
/*
 * Find the position of a smaller image in a larger one
 * when we know that the central pixel of the smaller
 * is in position (xc, yc) of the larger one. 
 * In order to undertand the outputs, consider the two 
 * points, we want their coordinates in both images: 
 * (1) The bottom left corner of the smaller array.
 * (2) The top right corner of the smaller array.
 * Don't forget that we are assuming the array element [0]
 * is positioned on the bottom left.
